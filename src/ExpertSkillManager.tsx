...
import { listExpertSkillUploads, type ExpertSkillConfirmationRecord, type ExpertSkillUploadRecord } from "./api/knowledge";
...
export function ExpertSkillManager(props: {
  ...
  async function reload() {
    setLoading(true);
    try {
      setUploads(await listExpertSkillUploads());
    } catch (caught) {
    ...
    <>
      <section className="expert-skill-manager-panel">
        ...
        <div className="expert-skill-manager-actions">
          <button className="primary-button" type="button" onClick={() => { setResumeUpload(null); setWizardOpen(true); }}>
        ...
        <div className="expert-skill-draft-list">
          ...
          {drafts.length ? drafts.slice(0, 5).map((upload) => (
            <button key={upload.id} type="button" onClick={() => { setResumeUpload(upload); setWizardOpen(true); }}>
      ...
      {wizardOpen && (
        <ExpertSkillWizard
          ...
          onClose={() => { setWizardOpen(false); setResumeUpload(null); void reload(); }}
