...
const DeferredLoginThreeScene = lazy(() =>
  import("./threeVisuals").then((module) => ({ default: module.LoginThreeScene })),
);
const DeferredAppThreeBackdrop = lazy(() =>
  import("./threeVisuals").then((module) => ({ default: module.AppThreeBackdrop })),
);

function useDeferredThreeVisuals() {
...
function LoginThreeScene() {
  const enabled = useDeferredThreeVisuals();
  const fallback = <div className="login-three-scene" aria-hidden="true" />;
  if (!enabled) return fallback;

  return (
    <Suspense fallback={fallback}>
      <DeferredLoginThreeScene />
    </Suspense>
  );
}
...
function AppThreeBackdrop(props: { role: Role }) {
  const enabled = useDeferredThreeVisuals();
  const fallback = <div className={`app-three-backdrop app-three-backdrop-${props.role}`} aria-hidden="true" />;
  if (!enabled) return fallback;

  return (
    <Suspense fallback={fallback}>
      <DeferredAppThreeBackdrop role={props.role} />
    </Suspense>
  );
}
