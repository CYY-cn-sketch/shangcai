...
public class StudentArtifactController {
    ...
    @PostMapping(value = "/artifacts/{artifactId}/pptx", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ArtifactApiModels.ArtifactResponse uploadPptx(
            Authentication authentication,
            @PathVariable String artifactId,
            @RequestParam("file") MultipartFile file
    ) {
        return ArtifactApiModels.ArtifactResponse.from(
                artifactService.attachPptxFile(authentication.getName(), artifactId, file),
                objectMapper
        );
    }
